...
SELECT @CMD = COALESCE( @CMD + Param_Name + '' = '' + 
							...
							+ '','' , ''''  ) 
  FROM MaintParameter
 WHERE Param_Job = ''DatabaseIntegrityCheck - SYSTEM_DATABASES''
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'IntegrityCheck - SYSTEM_DATABASES', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
...
EXEC msdb..sp_add_job 
		...
		@description = N'Pickup DBs not having run in-place DBCC CHECKDB for DB <2TB',
		@enabled=1,
		@notify_level_eventlog=3,
		@notify_level_email = 2,  
	    @notify_email_operator_name = N'SQLSupport', 
...
EXEC msdb.dbo.sp_add_jobstep @job_name='DatabaseIntegrityCheck - USER_DATABASES Pickup', @step_name=N'ListDbsSmallerThan', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=3, 
		@on_fail_action=2, 
...
GO
------------------------------------------------------
--  DatabaseIntegrityCheck - USER_DATABASES > 2TB   --
------------------------------------------------------
DECLARE @SchID INT
DECLARE @JobID BINARY(16)
SET @JobID = NULL 
...
SELECT @JobID = job_id 
FROM msdb.dbo.sysjobs 
WHERE ([Name] = N'DatabaseIntegrityCheck - USER_DATABASES > 2TB')
...
EXEC msdb..sp_add_job 
		@job_name = N'DatabaseIntegrityCheck - USER_DATABASES > 2TB',
...
EXEC msdb.dbo.sp_add_jobstep @job_name='DatabaseIntegrityCheck - USER_DATABASES > 2TB', @step_name=N'ListDbsLargerThan', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=3, 
		@on_fail_action=2, 
...
DECLARE @DaysBetween TINYINT = 14
DECLARE @DBThreshGB INT = 2048
...
EXEC msdb.dbo.sp_add_jobstep @job_id=@JobID, 
		@step_name=N'DatabaseIntegrityCheck - USER_DATABASES > 2TB', 
		@step_id=2,  
		@on_success_action=1, 
		@on_fail_action=2, 
		@retry_attempts=0, 
...
EXEC msdb..sp_add_job 
		...
		@description = N'In-place DBCC CHECKDB for DB <2TB',
		@enabled=1,
		@notify_level_eventlog=3,
		@notify_level_email = 2,  
	    @notify_email_operator_name = N'SQLSupport', 
...
UPDATE DBASupport.dbo.MaintParameter SET Param_Value='1080'  -- 16 hours for Sat/Sun runs
